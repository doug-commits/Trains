...
/* ------------------------------------------------ something to actually index */
{
  ...
  /* Every page names a share card, and the card it names exists.
   *
   * The site declared `twitter:card = summary_large_image` on every page and
   * supplied no image on any of them, so every share — X, LinkedIn, Facebook,
   * WhatsApp, Slack, Discord, Reddit — rendered as a blank rectangle with a
   * headline under it. Markup that promises a picture and has none does not
   * read as plain; it reads as broken, and a broken card is not passed on.
   * Nothing caught it because nothing looked.
   *
   * The existence half matters as much as the tag half: a card tag pointing at
   * a 404 is worse than no tag, and the images are generated by a browser and
   * committed, so a new route added without re-running tools/build-og.mjs
   * would otherwise ship exactly that. */
  {
    ...
    for (const f of [...pages, 'index.html']) {
      const html = readFileSync(
        f === 'index.html' ? join(root, 'index.html') : join(pub, f), 'utf8')
      const og = html.match(/<meta property="og:image" content="([^"]+)"/)
      const tw = html.match(/<meta name="twitter:image" content="([^"]+)"/)
      if (!og || !tw || og[1] !== tw[1]) { bad.push(f); continue }
      named.set(f, og[1])
    }
    check('every page names a share card, and names the same one twice',
      bad.length === 0, bad.slice(0, 5).join(' ') || `${named.size} pages`)

    const missing = [...new Set(named.values())]
      .map(u => u.replace(/^https?:\/\/[^/]+\/og\//, ''))
      .filter(f => !existsSync(join(root, 'data/og', f)))
    check('and the card it names has actually been rendered',
      missing.length === 0, missing.slice(0, 5).join(' ') || 'all present')

    /* A route page must show its own journey. Falling back to the default is
       the correct behaviour for a crossing and a silent downgrade for a route,
    ...
    check('and a route page shows its own route, not the general card',
      generic.length === 0, generic.slice(0, 5).join(' ') || 'all specific')

    // 1200×630 is what Facebook and X both ask for; a card at another size is
    // cropped by each of them differently.
    const odd = readdirSync(join(root, 'data/og')).filter(f => {
      const b = readFileSync(join(root, 'data/og', f))
      // JPEG SOF0/SOF2 marker carries the dimensions.
      for (let i = 2; i < b.length - 9; ) {
        if (b[i] !== 0xff) { i++; continue }
        const m = b[i + 1]
        if (m === 0xc0 || m === 0xc2) return b.readUInt16BE(i + 7) !== 1200 || b.readUInt16BE(i + 5) !== 630
        i += 2 + b.readUInt16BE(i + 2)
      }
      return true
    })
    check('and every card is the 1200x630 both networks ask for',
      odd.length === 0, odd.slice(0, 5).join(' ') || `${readdirSync(join(root, 'data/og')).length} cards`)
  }
