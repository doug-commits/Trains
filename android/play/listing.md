...
All regenerated by `node tools/store-assets.mjs` (and `tools/play-icon.mjs`
...
The tablet set is the same eight, in the same order, taken on a 600×960 dp
screen at 2× — a Nexus 7, and still the shape that slot means. That is under
the app's 60rem breakpoint, so a 7-inch tablet in portrait gets the same
full-screen map and pull-up sheet a phone does. The screenshots show that
because it is what installs, not because they were taken on the wrong device.
